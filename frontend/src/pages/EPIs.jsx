...
export default function EPIs() {
  ...
  // ✅ NOVO: Criar lote
  const [loteCreateOpen, setLoteCreateOpen] = useState(false)
  const [loteCreateItem, setLoteCreateItem] = useState({
    lote: '',
    nf_numero: '',
    nf_serie: '',
    fornecedor_cnpj: '',
  ...
  // ===== NOVO: LOTES (listar / criar / editar / excluir) =====
  ...
  const openLoteCreate = () => {
    ...
    setLoteCreateOpen(true)
  }

  // ✅ NOVO: salvar criação (POST)
  const saveLoteCreate = async () => {
    try {
      if (!lotesEpi?.id) {
        setSnack({ open: true, msg: 'Selecione um EPI para criar o lote.', sev: 'warning' })
        return
      }

      const payload = {
        epi: Number(lotesEpi.id),
        lote: loteCreateItem.lote || '',
        nf_numero: loteCreateItem.nf_numero || '',
        nf_serie: loteCreateItem.nf_serie || '',
        fornecedor_cnpj: loteCreateItem.fornecedor_cnpj || '',
        data_compra: loteCreateItem.data_compra || null,
        quantidade: Number(loteCreateItem.quantidade || 0),
        local_armazenamento: loteCreateItem.local_armazenamento || ''
      }

      await api.post('/estoques/', payload)

      setSnack({ open: true, msg: 'Lote cadastrado com sucesso.', sev: 'success' })
      setLoteCreateOpen(false)

      if (lotesEpi) await fetchLotes(lotesEpi)
    } catch (e) {
      setSnack({ open: true, msg: 'Falha ao cadastrar lote', sev: 'error' })
    }
  }
  ...
  return (
    ...
    >
      <Container sx={{ mt: 3 }}>
                ...
                >
                  <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.2} alignItems={{ xs: 'flex-start', md: 'center' }} justifyContent="space-between">
                    ...
                    <Stack direction="row" spacing={0.5} alignItems="center">
                      <Tooltip title="Gerenciar lotes (criar/editar/excluir)">
        ...
        <Dialog open={lotesOpen} onClose={() => setLotesOpen(false)} maxWidth="md" fullWidth>
          <DialogTitle sx={{ fontWeight: 900 }}>
            <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ xs: 'flex-start', sm: 'center' }} justifyContent="space-between" spacing={1}>
              ...
              {/* ✅ NOVO: botão para criar lote mantendo o designer */}
              <Button
                variant="contained"
                onClick={openLoteCreate}
        ...
        {/* ✅ NOVO Dialog: Criar Lote */}
        <Dialog open={loteCreateOpen} onClose={() => setLoteCreateOpen(false)} maxWidth="md" fullWidth>
          <DialogTitle sx={{ fontWeight: 900 }}>Novo lote</DialogTitle>
          <DialogContent dividers>
            <Grid container spacing={2} sx={{ mt: 0.5 }}>
              <Grid item xs={12} md={3}>
                <TextField label="Lote" fullWidth value={loteCreateItem.lote} onChange={e => setLoteCreateItem({ ...loteCreateItem, lote: e.target.value })} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }} />
              </Grid>
              <Grid item xs={12} md={3}>
                <TextField label="NF - Número" fullWidth value={loteCreateItem.nf_numero} onChange={e => setLoteCreateItem({ ...loteCreateItem, nf_numero: e.target.value })} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }} />
              </Grid>
              <Grid item xs={12} md={2}>
                <TextField label="NF - Série" fullWidth value={loteCreateItem.nf_serie} onChange={e => setLoteCreateItem({ ...loteCreateItem, nf_serie: e.target.value })} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }} />
              </Grid>
              <Grid item xs={12} md={4}>
                <TextField label="Fornecedor (CNPJ)" fullWidth value={loteCreateItem.fornecedor_cnpj} onChange={e => setLoteCreateItem({ ...loteCreateItem, fornecedor_cnpj: e.target.value })} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }} />
              </Grid>

              <Grid item xs={12} md={3}>
                <TextField label="Data da compra" type="date" fullWidth value={loteCreateItem.data_compra} onChange={e => setLoteCreateItem({ ...loteCreateItem, data_compra: e.target.value })} InputLabelProps={{ shrink: true }} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }} />
              </Grid>
              <Grid item xs={12} md={3}>
                <TextField label="Quantidade" type="number" fullWidth value={loteCreateItem.quantidade} onChange={e => setLoteCreateItem({ ...loteCreateItem, quantidade: e.target.value })} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }} />
              </Grid>
              <Grid item xs={12} md={6}>
                <TextField label="Local de armazenamento" fullWidth value={loteCreateItem.local_armazenamento} onChange={e => setLoteCreateItem({ ...loteCreateItem, local_armazenamento: e.target.value })} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }} />
              </Grid>
            </Grid>
          </DialogContent>
          <DialogActions sx={{ p: 2 }}>
            <Button onClick={() => setLoteCreateOpen(false)} startIcon={<CloseOutlinedIcon />} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 800 }}>
              Cancelar
            </Button>
            <Button variant="contained" onClick={saveLoteCreate} startIcon={<SaveOutlinedIcon />} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 900 }}>
              Salvar
            </Button>
          </DialogActions>
        </Dialog>
